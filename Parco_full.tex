...
Table \ref{tabMM} shows the case of $j$-$k$-$i$ order with outer loop parallelism. 
...
\begin{table}[htbp]
  \centering
  \caption{Execution time, Speed-up ratio, Performance, Upper bound of performance, and performance level for $j$-$k$-$i$ order of matrix multiplication}
  \label{tabMM}
  \small
...
Using AVX2 in this way, and using OpenMP to the outer loop, the performance can become the 67\% of the upper bound of performance.
